...
SRC_URI="mirror://sourceforge/${PN}/${PPP}.tar.gz"
...
DEPEND="media-video/mplayer
		...
		!gecko-sdk? ( || ( www-client/seamonkey www-client/mozilla-firefox ) )
		|| ( ( x11-libs/libXpm
				x11-proto/xextproto
			)
			virtual/x11
...
src_unpack() {
	unpack ${A}
	cd ${S}
	epatch ${FILESDIR}/${PN/-bin/}-gcc4.patch
	epatch ${FILESDIR}/mplayer-bin.patch
}
